...
namespace NinjaTrader.NinjaScript.Indicators
{
	public class GexHeatmap : Indicator
	{
		...
		public class GexBarData
		{
			...
			// Stores Calculated Real-Time Metrics for Tooltips
		...
		private class GexHitInfo
		{
			...
			public double Strike; // This is the Adjusted Future Strike
			public double NativeStrike; // The original Cash Strike
			public double Value;
			public int TotalOI;
		...
		[NinjaScriptProperty]
		[Display(Name="Live VIX Symbol", Description="Symbol for your real-time VIX feed (e.g. ^VIX or VX)", Order=2, GroupName="1. Data")]
		...
		protected override void OnStateChange()
		{
			...
			{
				Description					= @"Professional Real-Time Options Pricing & GEX Engine.";
			...
			else if (State == State.Configure)
			{
				...
				// Add secondary data series for Real-Time VIX polling
			...
			{
				if (ChartControl != null && isMouseSubscribed) ChartControl.MouseMove -= OnChartMouseMove;
				if (hoverTimer != null) hoverTimer.Stop();
				if (fileCheckTimer != null) fileCheckTimer.Stop();
				DisposeBrushes();
			}
		}
		...
		private string GetMappedTicker()
		{
			...
			if (name.Contains("RTY") || name.Contains("M2K")) return "RUT";
			if (name.Contains("YM")) return "DJI";
			return name;
		}
				...
				foreach (string file in files)
				{
					...
					try
					{
						...
						using (StreamReader sr = new StreamReader(fs))
						{
							...
							{
								string[] c = line.Split(',');
								if (c.Length < 8) continue; // Note: Length is now 8 due to VIX
								...
								try 
								{
									...
									if (!temp.ContainsKey(ts)) temp[ts] = new GexSnapshot { Timestamp = ts, BasisRatio = basis, SnapshotVIX = snapVix };
									if (!temp[ts].Strikes.ContainsKey(strike)) temp[ts].Strikes[strike] = new List<OptionData>();

									temp[ts].Strikes[strike].Add(new OptionData { ExpirationUtc = exp, IsCall = isCall, Strike = strike, OI = oi, BaseIV = iv });
								}
								catch { } 
							}
						}
		...
		{
			if (T <= 0 || v <= 0 || S <= 0 || K <= 0) return Math.Max(0, isCall ? S - K : K - S); // Intrinsic if expired
			double d1 = (Math.Log(S / K) + (r + v * v / 2.0) * T) / (v * Math.Sqrt(T));
		...
		{
			// Process only on the primary instrument (ES)
			...
			// Math Space vs Chart Space Calculations
			double basisRatio = snap.BasisRatio;
			double chartSpotPrice = Close[0]; 
			double nativeSpotPrice = chartSpotPrice / basisRatio; 

			// Dynamic Real-Time IV Scaling via VIX Proxy
			double vixRatio = 1.0;
			if (CurrentBars.Length > 1 && CurrentBars[1] >= 0)
			{
				double liveVix = Closes[1][0];
				if (snap.SnapshotVIX > 0 && liveVix > 0)
					vixRatio = liveVix / snap.SnapshotVIX;
			}

			// Strike Interval Mapping
			var sorted = snap.Strikes.Keys.OrderBy(k => k).ToList();
			double barInterval = 5.0 * basisRatio;
			for (int i = 1; i < sorted.Count; i++)
			{
				double diff = Math.Round((sorted[i] - sorted[i - 1]) * basisRatio, 2);
				if (diff > 0.1) { barInterval = diff; break; }
			}
			...
			foreach (var kvp in snap.Strikes)
			{
				...
				foreach (var opt in kvp.Value)
				{
					...
					// Apply Real-Time Market Fear dynamically
					double liveIV = opt.BaseIV * vixRatio;
					
					// Core calculations using True Native Space
					double gamma = CalculateGamma(nativeSpotPrice, nativeStrike, T, liveIV);
					double price = CalculateOptionPrice(nativeSpotPrice, nativeStrike, T, liveIV, opt.IsCall);
		...
		private System.Windows.Media.Color InterpolateColor(System.Windows.Media.Color c1, System.Windows.Media.Color c2, double r)
		{
			return System.Windows.Media.Color.FromRgb((byte)(c1.R + (c2.R - c1.R) * r), (byte)(c1.G + (c2.G - c1.G) * r), (byte)(c1.B + (c2.B - c1.B) * r));
		}
		...
		protected override void OnRender(ChartControl cc, ChartScale cs)
		{
			...
			for (int i = ChartBars.FromIndex; i <= ChartBars.ToIndex; i++)
			{
				...
				foreach (var kvp in dict)
				{
					...
					hits.Add(new GexHitInfo { Bounds = rect, Strike = kvp.Key, NativeStrike = kvp.Key / GetClosestSnapshot(Time[0]).BasisRatio, Value = kvp.Value, TotalOI = d.StrikeToTotalOI[kvp.Key], CallPrice = d.StrikeToCallPrice[kvp.Key], PutPrice = d.StrikeToPutPrice[kvp.Key], LiveIV = d.StrikeToLiveIV[kvp.Key] });
				}
			}
		...
		private void DrawTooltip(GexHitInfo info)
		{
			...
			string txt = string.Format(
				...
				"─────────────────────────\n" +
				"Original SPX Strike: {0:N0}\n" +
				"Total Option OI:     {1:N0}\n" +
				"Current Live IV:     {2:P1}\n" +
				"Est. Call Premium:   ${3:N2}\n" +
				"Est. Put Premium:    ${4:N2}\n\n" +
				"Rendered GEX Val:    {5:N2}", 
				info.NativeStrike, info.TotalOI, info.LiveIV, info.CallPrice, info.PutPrice, info.Value);

			using (var lay = new SharpDX.DirectWrite.TextLayout(NinjaTrader.Core.Globals.DirectWriteFactory, txt, tooltipTextFormat, 300, 200))
			{
